...
In this case, the last two charts are almost identical, but that's not always going to be the result.
...
Before I move on, a note on colour.  Usually I wouldn't use so many palettes in a single artefact for basically the same purpose.  The effect is (to me) somewhat jarring.  But as I'm demonstrating use of a new graphics function some of the normal requirements of good aesthetics are suspended.
