...
tag: 
   ...
   - Timeseries
   - R
description: X13-SEATS-ARIMA does not perform particularly well as an automated forecasting tool against the M3 forecasting competition data.  For 413 series it cannot fit a model without manual intervention.  When it does fit a model it has a tendency to give unrealistic forecasts based when it automatically chooses logarithm transform.
...
1. confirm Hyndman's results in his ["show me the evidence"](http://robjhyndman.com/hyndsight/show-me-the-evidence/) blog, showing that a combination of auto.arima() and ets() gives very good out-of-the-box forecasting results
...
As it turned out, the most interesting part of this was item 2.

To do all this I modified Hyndman's benchmarking code to add five additional forecasting methods:
...
This exercise ended up much more complex than I'd envisaged, and there's too much code to integrate into the text, so the full set of code can be found at .....

### X13-SEATS-ARIMA
[X13-SEATS-ARIMA](https://www.census.gov/srd/www/x13as/) is the industry standard tool for seasonal adjustment, particularly for official statistics published by national statistics offices.  It gives a choice of the SEATS algorithm from the Spanish stats office, and X11 (originally Canadian).  X13-SEATS-ARIMA combines these into a single software application, available for download from the US Census Bureau.  Christoph Sax's excellent [{seasonal} R package](https://cran.r-project.org/web/packages/seasonal/index.html) gives easy access from R.
...
* hybrids of methods - almost any methods - perform well, and the best result obtained by me in fact was an average of the forecasts of auto.arima(), ets() and theta().
...
There are five types of error that lead to X13 failing to fit a model.  Each error message is shown below with a minimal example to produce it.
